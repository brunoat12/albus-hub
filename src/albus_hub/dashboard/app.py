...
# O dashboard operacional consome a camada serving do Azure MySQL.
# Power BI fica responsável pela exploração histórica mais ampla.
...
# A versão visual original utilizava a Silver local para análises adicionais.
# No dashboard operacional final evitamos essa dependência.
incidents = None
...
try:
    ...
    predictions = current_predictions if not current_predictions.empty else None
except SQLAlchemyError as exc:
...
try:
    current_scores = load_current_risk_scores()
    risk_scores = current_scores if not current_scores.empty else None
except SQLAlchemyError as exc:
    risk_error = str(exc)
...
# Relatórios locais ficam opcionais. A aba de qualidade será removida
# na versão final do Streamlit, pois não faz parte da visão operacional.
ingestion_report_path = Path("__albus_missing_ingestion_report__.json")
gold_report_path = Path("__albus_missing_gold_report__.json")
...
default_period_index = (
    list(PERIOD_PRESETS).index(PERIOD_SLUGS[url_period]) if url_period in PERIOD_SLUGS else 1
)
...
tab_ops, tab_alerts, tab_forecast, tab_risk = st.tabs(["Operação", "Alertas", "Previsões", "Risco"])
...
with tab_ops:
    ...
    predicted_next = (
        float(next_prediction["predicted_incident_count"]) if next_prediction is not None else None
    )
    ...
    # Violações de OLA já estão consolidadas na Gold serving do MySQL.
    ola_breaches = scope_sum(
        daily_period,
        priority_scope,
        "kpi_breach_count",
    )
    ...
    history = daily_period.loc[daily_period["priority_scope"].eq(priority_scope)].sort_values(
        "reference_date"
    )
...
with tab_alerts:
    ...
    # Violações recentes calculadas diretamente pela Gold do MySQL.
    recent_end = daily_volume["reference_date"].max()
    recent_start = recent_end - pd.Timedelta(days=6)

    recent_daily = filter_period(
        daily_volume,
        "reference_date",
        recent_start,
        recent_end,
    ...
    if recent_breaches:
        alerts.append(
            (
                ...
                f"Janela {recent_start:%d/%m/%Y} — {recent_end:%d/%m/%Y}.",
            )
        )
